...
import { projectsDataFr } from "@/lib/data";
import Image from "next/image";
import { motion, useScroll, useTransform } from "framer-motion";
import { useRouter } from "next/navigation";

type ProjectProps = (typeof projectsDataFr)[number];
...
export default function Project({
  ...
  return (
    <motion.div
      ...
      className="group m-2 sm:m-3 last:mb-0"
    >
      <section className="bg-gray-100 max-w-[38rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem] hover:bg-gray-200 transition sm:group-even:pl-8 dark:text-white dark:bg-white/10 dark:hover:bg-white/20">
        <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-8 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[16rem]">
          {href ? (
            <a
              href={href}
              target="_blank"
